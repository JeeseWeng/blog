...
2. v-for 和 v-if 不要在同一个标签中使用,因为解析时先解析 v-for 再解析 v-if。 哪怕只渲染一小部分元素，也得在每次重新渲染的时候遍历整个列表，不论 active 是否发生了变化。如果 list 的数据有很多，就会造成性能低，页面可能卡顿的现象出现。

解决方法：

1. 如果遇到需要同时使用时可以考虑写成计算属性的方式。
